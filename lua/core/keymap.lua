...
local function buf_map(buf)
	...
	if not fn then
		function fn(mode, lhs, rhs, opts) buf_set_keymap(buf, mode, lhs, rhs, opts) end
...
local function resolve_mode(key)
	...
	for char in key:gmatch('.') do
		if not char:find('[!abcilnostvx]') then error(('invalid mode "%s"'):format(char)) end
		modes[char] = true
	end
	...
	if modes.x and modes.s then modes.v = true end
	if modes.n and modes.v and modes.o then modes[''] = true end
	if modes.i and modes.c then modes['!'] = true end
...
local function index(self, key)
	...
	if key == 'cmd' then
		local cmd_fn = function(str) return '<Cmd>' .. str .. '<CR>' end
	...
	local function map_fn(arg1, arg2, arg3)
		...
		if not maps then
			if type(arg2) == 'function' then
				opts.callback = arg2
				arg2 = ''
				if opts.expr and replace_keycodes == nil then opts.replace_keycodes = true end
			else
				opts.callback = nil
			end
			...
			for lhs, rhs in pairs(maps) do
				...
				elseif type(rhs) == 'string' then
					opts.callback = nil
					if opts.expr and replace_keycodes == nil then opts.replace_keycodes = false end
				else
